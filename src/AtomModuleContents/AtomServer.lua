...
Replacement of my team's framework, Origin, from 2019 to 2021.
...
local Services : Folder = AtomRoot.Services
...
--[[
Clean unused Instances and variables.

function YourFunction(YourFunctionsParameters)
...
function AtomMain.Start()
	if _VERSION ~= "Luau" then 
		ErrorSignal:Fire("Atom can't run on non-Luau based runtimes.") 
		return 
	end
	...
	return Promise.new(function(resolve)
		...
		for _, service in ipairs(Services:GetDescendants()) do
			if service:IsA("ModuleScript") and service:GetAttribute("Load") == true then
				table.insert(
		...
		print("Atom Started with a tick of " .. InitTick ..".")
	end)
end
